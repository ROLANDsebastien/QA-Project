...
export function getOptimizedImageUrl(
  ...
  try {
    ...
    if (!url.pathname.match(/\.(jpe?g|png|gif|svg|webp|avif)$/i)) {
      return src;
    }

    // Ajouter les paramètres d'optimisation à l'URL seulement si des options sont fournies
...
export function createResponsiveImageSet(
  ...
  return breakpoints
    .map(
      (width) =>
        `${getOptimizedImageUrl(src, { width, format: "webp" })} ${width}w`,
    )
    .join(", ");
}
